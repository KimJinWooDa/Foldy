    ...
    public static class AssetOrganizerCache
    {
        ...
        public static void AddPaths(IEnumerable<string> paths)
        {
            ...
            lock (lockObject)
            {
                ...
                if (hasChanges)
                {
                    ...
                    Debug.Log(
                        $"[AssetOrganizer] 📥 Added {paths.Count()} new assets to queue. Total: {pendingAssetPaths.Count}");
                }
            }
        }
        ...
        public static void RemovePaths(IEnumerable<string> paths)
        {
            ...
            lock (lockObject)
            {
                ...
                if (removedCount > 0)
                {
                    OnPendingAssetsChanged?.Invoke(pendingAssetPaths.ToList());
                    Debug.Log(
                        $"[AssetOrganizer] ✅ Processed {removedCount} assets. Remaining: {pendingAssetPaths.Count}");
                }
            }
        }
        ...
        public static void UpdateProjectStats(int processedAssets, int renamedAssets, int movedAssets)
        {
            lock (lockObject)
            {
                ...
                Debug.Log(
                    $"[AssetOrganizer] 📊 Updated stats: {processedAssets} processed, {renamedAssets} renamed, {movedAssets} moved");
            }
        }
