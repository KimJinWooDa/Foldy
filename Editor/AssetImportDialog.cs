...
namespace Foldy
{
    ...
    public class AssetImportDialog : EditorWindow
    {
        ...
        public static void ShowForAssets(List<string> assetPaths)
        {
            ...
            window.position = new Rect(center.x - WINDOW_WIDTH / 2, center.y - WINDOW_HEIGHT / 2, WINDOW_WIDTH,
                WINDOW_HEIGHT);
        ...
        private void DrawAssetListPanel()
        {
            ...
            var filteredAssets = string.IsNullOrEmpty(quickSearchText)
                ? importedAssetPaths
                : importedAssetPaths.Where(p => Path.GetFileName(p).ToLower().Contains(quickSearchText.ToLower()))
                    .ToList();
        ...
        private void DrawFolderNode(FolderNode node)
        {
            ...
            var buttonStyle = new GUIStyle(GUI.skin.label)
            {
                ...
                normal =
                {
                    textColor = selectedFolderPath == node.path
                        ? StyleHelper.Colors.Primary
                        : StyleHelper.Colors.TextPrimary
                }
            };
        ...
        private void DrawSettingsPanel()
        {
            ...
            if (showAdvancedOptions)
            {
                ...
                if (selectedConvention != null)
                {
                    selectedConvention.removeSpecialChars =
                        EditorGUILayout.Toggle("특수문자 제거", selectedConvention.removeSpecialChars);
                    selectedConvention.preserveNumbers =
                        EditorGUILayout.Toggle("숫자 보존", selectedConvention.preserveNumbers);
                    selectedConvention.autoCapitalize =
                        EditorGUILayout.Toggle("자동 대문자화", selectedConvention.autoCapitalize);
                }
        ...
        private void DrawActionButtons()
        {
            ...
            GUILayout.BeginArea(new Rect(20, position.height - buttonAreaHeight + 10, position.width - 40,
                buttonAreaHeight - 20));
            EditorGUILayout.BeginHorizontal();
            ...
            foreach (var assetPath in importedAssetPaths)
            {
                try
                {
                    ...
                    if (assetPath != targetPath)
                    {
                        ...
                        var result =
                            new AssetProcessingResult(assetPath, targetPath, string.IsNullOrEmpty(error), error);
                        processingResults.Add(result);
